...
class ChatAgent:
    ...
    async def process_message(self, request: ChatRequest) -> ChatResponse:
        ...
        if session.state == ConversationState.GOODBYE:
            ...
            return self._response(
                session_id, session, "Novo atendimento iniciado. Como posso ajudar?"
            )

        if (
            any(w in normalized for w in GOODBYE_WORDS)
            and session.state != ConversationState.WELCOME
        ):
            session.state = ConversationState.GOODBYE
            return self._response(
                session_id, session, "Obrigado por usar o Banco Ágil! Até a próxima."
            )

        if session.state == ConversationState.WELCOME:
            return await self._handle_welcome(session_id, session, message, normalized)

        if session.state == ConversationState.COLLECTING_CPF:
            return await self._handle_cpf(session_id, session, message, normalized)

        if session.state == ConversationState.COLLECTING_BIRTHDATE:
            return await self._handle_birthdate(
                session_id, session, message, normalized
            )
        ...
        if session.state == ConversationState.WAITING_LIMIT_VALUE:
            return await self._handle_limit_value(session_id, session, message)

        if session.state == ConversationState.WAITING_CURRENCY:
            return await self._handle_currency(session_id, session, message)
        ...
        if session.state in [
            ...
            return await self._handle_interview(session_id, session, message)
    ...
    async def _handle_welcome(
        self, session_id: str, session: SessionData, message: str, normalized: str
    ) -> ChatResponse:
    ...
    async def _handle_cpf(
        ...
        if any(g in normalized for g in GREETINGS):
            return self._response(
                session_id, session, "Olá! 😊 Para continuar, preciso do seu CPF."
            )

        if any(
            w in normalized
            for w in ["nao sei", "esqueci", "nao lembro", "onde encontro"]
        ):
            return self._response(
                session_id,
                session,
        ...
        if cpf is None:
            if any(h in normalized for h in HELP_WORDS):
                return self._response(
                    session_id,
                    session,
                    "O CPF é um número de 11 dígitos. Você pode digitar com ou sem pontos e traço.\n\nExemplo: 123.456.789-01 ou 12345678901",
                )
            return self._response(
                session_id,
                session,
                "Não consegui identificar o CPF. Por favor, digite os 11 números.\n\nExemplo: 12345678901",
            )
        ...
        if len(cpf) != 11:
            return self._response(
                session_id,
                session,
                f"O CPF deve ter 11 dígitos. Você digitou {len(cpf)}. Tente novamente.",
            )
    ...
    async def _handle_birthdate(
        ...
        if any(g in normalized for g in GREETINGS):
            return self._response(
                session_id,
                session,
                "Oi! Estamos quase lá. Só preciso da sua data de nascimento para validar.",
            )
        ...
        if any(w in normalized for w in ["nao sei", "esqueci", "nao lembro"]):
            return self._response(
                session_id,
                session,
                "A data de nascimento é necessária para sua segurança. Você pode verificar em um documento como RG ou CNH.",
            )
        ...
        if date_parts is None:
            return self._response(
                session_id,
                session,
                "Não entendi a data. Por favor, informe no formato dia/mês/ano.\n\nExemplo: 15/05/1990",
            )
        ...
        if year < 1900 or year > current_year:
            return self._response(
                session_id,
                session,
                f"O ano parece incorreto. Deve estar entre 1900 e {current_year}.",
            )
        ...
        try:
            ...
            return self._response(
                session_id,
                session,
                "Data inválida. Verifique se o dia existe no mês informado.",
            )
    ...
    async def _handle_authenticated(
        ...
        if any(
            w in normalized
            for w in ["limite", "credito", "saldo", "quanto tenho", "meu limite"]
        ):
            return await self._get_credit_limit(session_id, session)

        if any(
            w in normalized
            for w in ["aumento", "aumentar", "mais limite", "elevar", "subir"]
        ):
            ...
            return self._response(
                session_id,
                session,
                "Qual valor de limite você gostaria?\n\nPode digitar: 25000, 25k, ou vinte e cinco mil",
                authenticated=True,
                token=session.token,
            )
        ...
        if any(
            w in normalized
            for w in ["cambio", "dolar", "euro", "moeda", "cotacao", "libra", "iene"]
        ):
        ...
        if intent == "request_increase":
            ...
            return self._response(
                session_id,
                session,
                "Entendi que você quer aumentar o limite. Qual valor?",
                authenticated=True,
                token=session.token,
            )
        if intent == "exchange_rate":
            ...
            return self._response(
                session_id,
                session,
                "Entendi! Qual moeda você quer consultar?",
                authenticated=True,
                token=session.token,
            )
        if intent == "interview":
    ...
    async def _handle_limit_value(
        self, session_id: str, session: SessionData, message: str
    ) -> ChatResponse:
        value, error_msg = self._parser.parse_limit_value(message)
        if value is None:
            return self._response(
                session_id, session, error_msg, authenticated=True, token=session.token
            )
        session.state = ConversationState.AUTHENTICATED
        return await self._request_limit_increase(session_id, session, value)
    ...
    async def _get_credit_limit(
        ...
        return self._response(
            ...
            data={
                "score": result.score,
                "limit": result.current_limit,
                "available": result.available_limit,
            },
        )

    async def _request_limit_increase(
        self, session_id: str, session: SessionData, value: float
    ) -> ChatResponse:
        from src.agents.credito import CreditAgent
        ...
        return self._response(
            ...
            authenticated=True,
            token=session.token,
            data={"status": result.status, "requested": value},
        )

    async def _get_exchange_rate(
        self, session_id: str, session: SessionData, currency: str
    ) -> ChatResponse:
        ...
        return self._response(
            ...
            authenticated=True,
            token=session.token,
            data={"rate": result.rate, "currency": currency},
        )
    ...
    def _start_interview(self, session_id: str, session: SessionData) -> ChatResponse:
        ...
        return self._response(
            session_id,
            session,
            "Vamos atualizar seu perfil financeiro! Isso pode melhorar seu score.\n\n**Qual é sua renda mensal?**\n\n(Pode digitar: 6000, 6k, ou seis mil)",
            authenticated=True,
            token=session.token,
        )
    ...
    async def _handle_interview(
        ...
        if state == ConversationState.INTERVIEW_INCOME:
            ...
            return self._response(
                session_id,
                session,
                f"Renda: **R$ {value:,.2f}**\n\n**Qual seu tipo de trabalho?**\n\n(CLT, autônomo, MEI, servidor público, desempregado)",
                authenticated=True,
                token=session.token,
            )
        ...
        if state == ConversationState.INTERVIEW_EMPLOYMENT:
            ...
            return self._response(
                session_id,
                session,
                f"Tipo: **{names.get(emp, emp)}**\n\n**Qual o total de despesas mensais?**\n\n(aluguel, contas, alimentação, etc)",
                authenticated=True,
                token=session.token,
            )
        ...
        if state == ConversationState.INTERVIEW_EXPENSES:
            ...
            return self._response(
                session_id,
                session,
                f"Despesas: **R$ {value:,.2f}**\n\n**Quantos dependentes você tem?**\n\n(filhos, cônjuge sem renda, pais)",
                authenticated=True,
                token=session.token,
            )
...
**Confirma esses dados?** (sim ou não)"""
            return self._response(
                session_id, session, summary, authenticated=True, token=session.token
            )
        ...
        if state == ConversationState.INTERVIEW_CONFIRM:
            ...
            if confirmed is None:
                return self._response(
                    session_id,
                    session,
                    "Não entendi. Os dados estão corretos? (sim ou não)",
                    authenticated=True,
                    token=session.token,
                )
            if not confirmed:
        ...
        return self._response(
            session_id,
            session,
            "Algo deu errado na entrevista. Digite 'atualizar perfil' para recomeçar.",
            authenticated=True,
            token=session.token,
        )
